# CEREMOS 開発引き継ぎメモ（新しいタスク用）

結婚式場・宴会場向けERP「CEREMOS」。このファイルを読めば前セッションの続きから作業できる。

## プロジェクト概要
- Next.js 14.2 App Router / Prisma 5.22 + SQLite / JWT(jose) / bcryptjs
- 場所: `~/Documents/wedding-erp`（このフォルダ。旧: ~/Desktop/wedding-erp）
- 本番: Sakura VPS https://os3-376-21308.vs.sakura.ne.jp（pm2 + Caddy HTTPS、app名 `wedding-erp`）
...
```
- **テンプレ移行（2026-07-06以降・初回のみ）**: `ssh ubuntu@… "cd ~/wedding-erp && node scripts/clear-templates.cjs --yes"`（旧内蔵テンプレを全削除。以後テンプレはAI生成JSONを管理画面から読み込む）
- （旧）refresh-templates.cjs は内蔵テンプレ廃止により実質「isSystem削除のみ」になった
- server-update.sh が `prisma db push` とビルド・pm2 restart を実行
- **注意: rsyncは削除を同期しない**。ローカルでファイルを削除したデプロイでは、サーバー側でも `ssh … "cd ~/wedding-erp && rm -rf <削除したパス>"` を実行してからビルドすること（残骸がビルドエラーになる。2026-07-06に旧認証APIの残骸で発生）
- ブラウザ確認は Cmd+Shift+R。タブタイトルに更新(ビルド)日時が出る（NEXT_PUBLIC_BUILD_AT）

## サンドボックスでの検証手順（Claude用・重要）
- Prisma生成: `/tmp/werp/` に package.json `{"name":"werp-check","private":true}`、node_modules を repo に symlink、schema を sed で `output = "/tmp/werp/client"` 追記して
...
- マウント先で rm/unlink 不可 → 廃止ファイルは Write で `export {};` スタブ化 or redirectページ化
- 未確定シェル: bashは毎回独立・cwd引き継ぎなし。絶対パスで

## 主要ファイルマップ
- `prisma/templates.cjs` … システムテンプレ本体（quote/rundownは分割ファイルをrequire）
- `prisma/quote-templates.cjs` … 見積テンプレ（ブライダル④〜⑩＋宴会③〜⑤。計15種）
- `prisma/rundown-templates.cjs` … 進行表テンプレ①〜⑪（司会台本・{新郎}{新婦}トークン・実所要分。⑪=宴会汎用）
- `src/app/api/v1/cases/[id]/quotes/route.ts` … 見積保存時の自動セットアップ（料理menu-presets／席次／リソース標準セット／進行表を名前推定 or rundownTemplateIdで適用。宴会系→⑪）
- `src/components/quotes-panel.tsx` … 見積UI。**新規=ウィザード**（Step1 ブライダル/宴会/前Ver/白紙 → Step2 プランカード）。編集中に進行表テンプレselect（自動推定 or 手動）
...
- `src/components/songs-panel.tsx` … 選曲専用（視聴まで）。🎲全曲おまかせ／♪全シーンに曲枠作成／⏱cueTiming
- `src/components/audio-console.tsx` … 黒ベース再生プレイヤー（Space/Esc・波形シーク・F.I/F.O・投影モニター・暗転・待機画像）
- `src/app/live/[id]/screen/page.tsx` … 映像ウィンドウ（YouTubeタイトル隠し: 開始6秒/シーク後4.5秒カバー・終了0.9秒前に待機画像へ・フル画面ボタン・キー転送handshake）
- BroadcastChannel名 `ceremos-live-${caseId}`（cmd: load/volume/seek/stop/fade/blackout/idle-refresh/fullscreen/progress/screen-ready/screen-ended/key）
- `src/app/login/page.tsx` … お客様入口。**ログイン/新規登録の2タブ・ID(メール)+パスワードのみ**（Google/マジックリンクUIは撤去済み、APIは残存）。?quick=トークン(24h)経由の登録は承認不要で即利用
- `src/app/api/v1/auth/signup/route.ts` … 通常登録=名前+メール+PWのみ・approved:false（プランナー承認待ち）。quick付き=approved:true
- `src/app/print/signup-qr/page.tsx` … お客様ログインQRポスター(A4・常設/login)
- `src/app/api/v1/branding/route.ts` … 公開ロゴAPI（ログイン画面用）
- `src/components/new-case-form.tsx` … 新規案件（進行表テンプレ欄は削除済み。進行表は見積テンプレから自動生成）。直近72hクイック登録客チップ・QR
- `src/components/settings-admin.tsx` … 会場・設備マスタ（実寸m）／ロゴ（推奨: 横長3:1〜4:1・幅900px+・透過PNG）／待機画像／QR説明
- `src/components/day-venue-chart.tsx` … 会場使用バーチャート（try/catchガード済み）
- `src/lib/freee.ts` + billing-panel … freee取引同期・CSV
- `scripts/refresh-templates.cjs --all` … isSystemテンプレ差し替え
