...
- `src/lib/template-pack.ts`: 新しいパック適用経路`applyPackToCase`が`caseType`を取得すらしておらず、宴会案件でもリソースタブの控室ラベルが常に「新郎新婦 控室」「親族控室」になっていた問題を修正（`isBridal(c.caseType)`で分岐し「主催者 控室」「スタッフ控室」に）。旧経路（`quotes/route.ts`）は既に対応済みだった
- `src/app/api/v1/catalog/public/route.ts` + `src/components/catalog-showcase.tsx`: 公開フル画面カタログ（`/catalog`）のウィザードに`caseType`判定が一切なく、宴会案件のお客様にも「👰新婦の衣装／🤵新郎の衣装」ステップが表示されていた問題を修正。APIがcoupleの案件`caseType`を返すよう拡張し、宴会案件では`BRIDAL_ONLY_CATALOG_CATEGORIES`（dress）を除外。残った美容小物のみのケースはステップ名を「衣装」→「小物・美容」に差し替え（スタッフ・未紐付け時は案件選択前で判別不可のため従来通りブライダル既定のまま＝対象外）
- `src/app/(app)/dashboard/page.tsx`: 「今やること」フィードで🔮ヒヤリング診断（Case.hearingJson）と📝事前アンケート（User.profileJson.survey）が別物と分からず並んで表示される軽微な紛らわしさを、アンケート側の文言に「診断とは別の項目です」を追記して緩和（データ自体の競合・上書きはなし）
- 検証：`npx tsc --noEmit`・`npm run build`通過。ローカルDEMO_MODEで既存の宴会案件（株式会社アゾン 50周年）にデモ顧客アカウントを一時的に紐付けてブラウザ実機確認（カタログウィザードの表紙フロー表示が「03 衣装」→「03 小物・美容」に変化、衣装ステップ内の新婦/新郎グループが非表示に）。検証用の一時紐付けは確認後にDB上で削除済み。ダッシュボードのアンケート文言変更も別アカウントで確認。席次表印刷とテンプレパック控室ラベルはシード済み案件に対象データ（ゲスト・リソース割当）がなく画面実写確認は未実施だが、同ファイル内の既存の正しい実装（見出し行・旧経路）と同一パターンのため妥当性は高い

## 2026-07-11（フル画面カタログに「アプリに戻る」導線を追加）
- `src/components/catalog-showcase.tsx`: 表紙・ウィザード中どちらにも「アプリに戻る」導線が一切なく、ログイン中のお客様/スタッフが`/catalog`（URL共有・新規プランウィザードから遷移）から抜けられない問題を修正。`viewer`がある場合のみ、`viewer.caseId`があれば案件詳細へ・なければダッシュボードへ戻るリンクをヘッダー（表紙は左上固定、ウィザード中はトップバー内）に追加
- `src/app/catalog/catalog.css`: `.cs-back`／`.cs-back-hero`のスタイルを追加
- 検証: ローカルでログイン中カップルとして`/catalog`にアクセスし、表紙に「← アプリに戻る」（href=案件詳細URL）が表示されることを確認。`npx tsc --noEmit`通過

## 2026-07-11（お客様の承認フローを復元・登録直後にヒヤリング10問へ誘導。案件の自動作成/自動紐付けは撤回）
直前のセッションで撤廃した「お客様承認」と「案件の自動作成」を、方針転換により元に戻した。
- `src/app/api/v1/auth/signup/route.ts`: 通常のお客様セルフ登録を`approved: false`に戻し、案件自動作成ロジックを削除（コメントも「プランナーが新規案件画面でこのお客様を選んで作成・紐付けする」に復元）。login/route.ts・(app)/layout.tsx・clock.ts・approvals/page.tsxの関連コメント/文言もあわせて復元
- `src/lib/survey.ts`: 30問の`SURVEY_30`を廃止し、`WEDDING_SURVEY`・`BANQUET_SURVEY`（各10問）に再設計。宴会用は「挙式スタイル」→「会の目的・種類」、「写真・映像の希望」→「音響・映像演出の希望」などブライダル特有の質問を宴会向けに差し替え。ラベル辞書用に両方を合わせた`SURVEY_ALL`を新設し、既存の参照箇所（cases/[id]/page.tsx・customers-admin.tsx・pending-approvals.tsx）を追従
- `src/app/api/v1/auth/survey/route.ts`: GETレスポンスに`eventType`を追加（プロフィールJSONのeventTypeから判定・宴会/ブライダルの出し分けに使用）
- `src/app/survey/page.tsx`: `eventType`に応じて`WEDDING_SURVEY`/`BANQUET_SURVEY`を出し分け。基本情報の「お相手のお名前」欄も宴会では「ご担当部署・役職」に変更。案内文を「登録直後のヒヤリング」の位置づけに更新
- `src/app/login/page.tsx`: 新規登録（お客様・ブライダル/宴会）完了後、自動ログイン＋ダッシュボード遷移ではなく`/survey?token=...`へ直行するように変更（トークンなので承認前でも回答可）。業者登録・クイックQR登録は従来どおり自動ログインを試行。登録直後の案内文もヒヤリングへの言及を追加
- 検証: ローカルでブライダル/宴会それぞれ新規登録→ヒヤリングページへ直行し質問セットが正しく出し分けられること、`approved=false`・案件が自動作成されていないことをDBで確認。回答の保存（POST）も確認。`npx tsc --noEmit`・`npm run build`通過

## 2026-07-11（お客様セルフ登録で仮案件を自動作成・プランナーの紐付け作業を撤廃／案件詳細ヒーローバー撤去）
- `src/app/api/v1/auth/signup/route.ts`: 通常のお客様セルフ登録（ブライダル・宴会）で、ユーザー作成と同時に開催日未定（半年後の仮日程）・仮予約ステータスのCaseを自動作成し、CaseMemberで即紐付け。plannerIdは未割当（null）のまま案件一覧に並ぶため、プランナーが案件を選んで手動リンクする作業が不要に。クイックQR登録・業者登録は対象外（従来通りスタッフが案件作成）
- `src/app/(app)/cases/[id]/page.tsx` + `globals.css`: 全タブに固定表示していたヒーローバー（日程詳細・次回打ち合わせ・見積書リンク）を撤去。各情報は📌案件／📝打ち合わせ／💰見積の各タブに既に重複表示されているため。「▶当日運営」リンクのみ開催前日以内の条件付きでタブバー要約行に移設
- `src/app/signup/complete/page.tsx`: QR登録後完了画面に残っていた「式場スタッフの承認が完了するとログインいただけます」という古い文言を「すぐにログインしてご利用いただけます」に修正し、ログイン画面への導線ボタンを追加（このページ自体は現行のQR登録フローから遷移しないため実害は無かったが、念のため修正）
- 検証: ローカルでcurl+ブラウザにより新規登録→自動作成された仮案件（ステータス仮予約・plannerId null）がお客様ダッシュボードと管理者の案件一覧の両方に正しく表示されることを確認。`npx tsc --noEmit`通過
