// AIテンプレ生成用プロンプト（テンプレ管理画面の「🤖 AIプロンプトをコピー」で使用）
// ChatGPT / Claude / Gemini などに貼り付けて、出力されたJSONを「📥 AIテンプレ読み込み」に貼るだけで使える。
...
// 🛍 カタログ（業者・品目・写真）のスキーマ。テンプレ一式と同じ配列に混ぜて読み込める
export const CATALOG_JSON_SCHEMA = `{
...
export function buildAiPrompt(): string {
  return `あなたは日本の結婚式場・宴会場のベテランプランナーです。
